...
- **Прозрачный перехват** через `iptables` DNAT - весь TCP :80/:443 уходит на
  прокси без компаньон-приложения и без VPN-диалога. Mockttp определяет цель по
  Host/SNI. Зависит от ядра эмулятора: loopback-DNAT (через reverse-туннель)
  доставляет не на всех ядрах - на части эмуляторных образов (напр. MEmu
  Android 9) локальный пакет, завёрнутый на `127.0.0.1`, не маршрутизируется
  обратно (reply-path), и перехват не идёт. На таких образах работает только
  `http_proxy`-режим. См. раздел про ограничения.
...
## Ограничение: proxy-aware vs proxy-ignoring приложения

`http_proxy`-режим ловит только приложения, которые **уважают системный proxy**
(на OkHttp и т.п. - например Spotify). Часть приложений шлёт API мимо системного
proxy своим сетевым стеком (например клиент VK) - такие через `http_proxy` не
перехватываются, а при выставленном proxy могут вообще терять сеть.

Полностью прозрачный перехват таких приложений (как делает VPN-режим HTTP
Toolkit) надёжно решается только через **VpnService** (tun-интерфейс + обработка
пакетов в userspace), который обходит маршрутные ограничения ядра. `iptables`-
DNAT - более лёгкая альтернатива, но, как описано выше, доставляет не на всех
эмуляторных ядрах. VPN-режим в этот инструмент пока не входит.
